...
| **DB-2** | Aurora PostgreSQL trails community PostgreSQL. `README.md:33` states `PostgreSQL >= 18.0` and `config/docker/compose/postgresql.yml` runs `postgres:18.3`. Is an Aurora PostgreSQL major version available that Fineract supports, and has the Liquibase changelog set (237 files) been validated against it? | If not, row 3 becomes an exception to guardrail G3 | WS3 | **P0** |
