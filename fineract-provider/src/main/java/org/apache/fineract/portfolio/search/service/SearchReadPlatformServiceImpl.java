...
public class SearchReadPlatformServiceImpl implements SearchReadPlatformService {
    ...
    public String searchSchema(final SearchConditions searchConditions) {
        ...
        final String clientMatchSql = "( (select 'CLIENT' as entityType, c.id as entityId, c.display_name as entityName, c.external_id as entityExternalId, c.account_no as entityAccountNo "
                ...
                + " from m_client c join m_office o on o.id = c.office_id where o.hierarchy like :hierarchy and (lower(c.account_no) like lower(:search) or lower(c.display_name) like lower(:search) or lower(c.external_id) like lower(:search) or lower(c.mobile_no) like lower(:search))) "
                + " order by c.id desc)";
        ...
        final String loanMatchSql = "( (select 'LOAN' as entityType, l.id as entityId, pl.name as entityName, l.external_id as entityExternalId, l.account_no as entityAccountNo "
                ...
                + " from m_loan l left join m_client c on l.client_id = c.id left join m_group g ON l.group_id = g.id left join m_office o on o.id = c.office_id left join m_product_loan pl on pl.id=l.product_id where (o.hierarchy IS NULL OR o.hierarchy like :hierarchy) and (lower(l.account_no) like lower(:search) or lower(l.external_id) like lower(:search))) "
                + " order by l.id desc)";
        ...
        final String savingMatchSql = "( (select 'SAVING' as entityType, s.id as entityId, sp.name as entityName, s.external_id as entityExternalId, s.account_no as entityAccountNo "
                ...
                + " where (o.hierarchy IS NULL OR o.hierarchy like :hierarchy) and (lower(s.account_no) like lower(:search) or lower(s.external_id) like lower(:search))) "
                + " order by s.id desc)";
        ...
        final String clientIdentifierMatchSql = "( (select 'CLIENTIDENTIFIER' as entityType, ci.id as entityId, ci.document_key as entityName, "
                ...
                + " where o.hierarchy like :hierarchy and lower(ci.document_key) like lower(:search) ) " + " order by ci.id desc)";
        ...
        final String groupMatchSql = "( (select CASE WHEN g.level_id=1 THEN 'CENTER' ELSE 'GROUP' END as entityType, g.id as entityId, g.display_name as entityName, g.external_id as entityExternalId, g.account_no as entityAccountNo, "
                ...
                + " from m_group g join m_office o on o.id = g.office_id where o.hierarchy like :hierarchy and (lower(g.account_no) like lower(:search) or lower(g.display_name) like lower(:search) or lower(g.external_id) like lower(:search) )) "
                + " order by g.id desc)";
